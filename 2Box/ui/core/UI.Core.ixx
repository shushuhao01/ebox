...
namespace ui
{
	...
	public:
		...
		// 悬停时是否显示“左右调整大小”光标（用于可拖动分割线）
		virtual bool wantsResizeCursor() const noexcept
		{
			return false;
		}
	...
	public:
		...
		ControlBase* hoveredControl() const noexcept { return m_currentHovered; }
	...
	protected:
		...
		// 在窗口最上层绘制全局悬浮提示
		void drawTooltip(const RenderContext& renderCtx);
	...
	// 简单的模态文本输入对话框，用于环境重命名等场景；取消或输入为空时返回 nullopt
	export std::optional<std::wstring> input_text(const WindowBase* owner, std::wstring_view title, std::wstring_view initial);

	// 清理环境数据对话框：三个选项（仅缓存/仅聊天记录/都清理），
	// 选中后提示对应清理内容，确认后返回选中项，取消/关闭返回 nullopt。
	export enum class ECleanOption : std::uint8_t
	{
		Cache,
		ChatData,
		Both,
	...
	export std::optional<ECleanOption> confirm_clean_dialog(const WindowBase* owner, std::wstring_view envName);

	// 激活码输入对话框：输入有效激活码后返回 true（已成功激活），取消/关闭/始终无效返回 false。
	// 内部直接校验激活码，无效时在对话框内提示，不弹出额外 MessageBox。
	export bool license_activation_dialog(const WindowBase* owner);

	// 授权信息对话框结果
	export enum class LicenseInfoResult
	{
		None,        // 直接关闭
		Reactivate,  // 点击“重新激活/续期”
		Unbound,     // 解绑成功（调用方应退出应用）
	};
