...
namespace ui
{
	...
	HResult WindowBase::onRender()
	{
		...
		// 悬浮提示绘制在最上层，不受各控件裁剪限制
		drawTooltip(ctx);
		return ctx.renderTarget->EndDraw();
	}

	void WindowBase::drawTooltip(const RenderContext& renderCtx)
	{
		if (!m_bTooltipVisible || m_tooltipText.empty())
		{
			return;
		}
		ID2D1HwndRenderTarget* const renderTarget = renderCtx.renderTarget;
		ID2D1SolidColorBrush* const solidBrush = renderCtx.brush;
		if (!renderTarget || !solidBrush)
		{
			return;
		}

		UniqueComPtr<IDWriteTextLayout> layout;
		if (FAILED(app().dWriteFactory()->CreateTextLayout(m_tooltipText.c_str(),
		                                                   static_cast<UINT32>(m_tooltipText.size()),
		                                                   app().textFormat().pTipsFormat,
		                                                   600.f, std::numeric_limits<float>::max(),
		                                                   &layout)))
		{
			return;
		}
		DWRITE_TEXT_METRICS textMetrics{};
		if (FAILED(layout->GetMetrics(&textMetrics)))
		{
			return;
		}
		const float tipWidth = textMetrics.width + 16.f;
		const float tipHeight = textMetrics.height + 8.f;
		float x = m_tooltipPos.x + 12.f;
		float y = m_tooltipPos.y + 16.f;

		if (RECT rc{}; GetClientRect(nativeHandle(), &rc))
		{
			const float clientW = static_cast<float>(rc.right) * dpiInfo().physicalToDevice;
			const float clientH = static_cast<float>(rc.bottom) * dpiInfo().physicalToDevice;
			if (x + tipWidth > clientW)
			{
				x = m_tooltipPos.x - tipWidth - 4.f;
			}
			if (y + tipHeight > clientH)
			{
				y = m_tooltipPos.y - tipHeight - 4.f;
			}
			if (x < 0.f)
			{
				x = 0.f;
		...
		renderTarget->DrawRoundedRectangle(roundedRect, solidBrush, 1.f);
		solidBrush->SetColor(D2D1::ColorF(0x333333));
		renderTarget->DrawTextLayout(D2D1::Point2F(x + 8.f, y + 4.f), layout, solidBrush);
	}
		...
		try
		{
			...
			else
			{
				if (WindowBase* pWnd = reinterpret_cast<WindowBase*>(GetWindowLongPtrW(hWnd, GWLP_USERDATA)))
				{
					...
					case WM_SETCURSOR:
						{
							// 悬停在可拖动分割线上时显示左右调整大小光标
							if (LOWORD(lParam) == HTCLIENT)
							{
								if (const ControlBase* ctrl = pWnd->m_controlManager.hoveredControl(); ctrl && ctrl->wantsResizeCursor())
								{
