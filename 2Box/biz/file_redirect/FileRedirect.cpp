...
namespace
{
	...
	// 源机企业微信正常运行时，其数据文件可能被独占，首次拷贝失败后等待重试
	...
	bool copy_file_with_retry(const std::wstring& originalFile, const std::wstring& redirectFile)
	{
		...
		for (int attempt = 0; attempt < maxRetries; ++attempt)
		{
			...
			catch (const fs::filesystem_error& e)
			{
				...
				OutputDebugStringW(std::format(L"[2Box] copy redirect file failed: {} -> {} (err={})\n",
				                               originalFile, redirectFile, err).c_str());
				return false;
			}
			catch (...)
			{
				OutputDebugStringW(std::format(L"[2Box] copy redirect file failed (unknown): {} -> {}\n",
