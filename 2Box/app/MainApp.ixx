...
public:
	...
	static constexpr std::wstring_view kServiceUrl{L""};     // 微信客服
...
public:
	...
	//  3) 全新部署：默认 C:\2BoxData（C 盘不可用则 D:\2BoxData），并写入防误删提示文件
	//  4) 兜底：exe 目录
	...
	std::wstring envDataRoot() const
	{
		...
		constexpr std::wstring_view regSubKey{L"Software\\2Box"};
		...
		// 1) 注册表已有记录且可写 → 直接使用
		{
			...
			{
				const LSTATUS st = RegQueryValueExW(hKey, regValueName.data(), nullptr, nullptr,
		...
		// 已注册过数据目录的老环境不受影响（走分支 1），环境数据随注册表持久化。
		...
		std::array<std::wstring_view, 2> candidateRoots{L"C:\\2BoxData", L"D:\\2BoxData"};
			...
			if (tryDir(rootStr))
			{
				...
				if (!fs::exists(tipFile, ec2) || ec2)
				{
					...
					if (ofs)
					{
						...
						ofs << "【重要】此文件夹是 2Box 多开环境的数据目录（存放企业微信等应用的环境数据）。\n"
...
private:
	...
	// 持久化环境数据根目录到 HKCU\Software\2Box\DataDir
	static void save_env_data_root(const std::wstring& dir)
	{
		HKEY hKey = nullptr;
		if (RegCreateKeyExW(HKEY_CURRENT_USER, L"Software\\2Box", 0, nullptr, 0, KEY_WRITE, nullptr, &hKey, nullptr) == ERROR_SUCCESS)
		{
