...
namespace rpc
{
	Server::Server()
	{
		RPC_STATUS status = RpcServerUseProtseqEpA(RPC_CSTR("ncalrpc"), RPC_C_LISTEN_MAX_CALLS_DEFAULT, RPC_CSTR("{63B40BDA-A2D1-4516-BDBB-E1E2A960D31E}eBoxServer"), nullptr);
		if (status != RPC_S_OK)
		{
			if (status == RPC_S_DUPLICATE_ENDPOINT)
			{
				MessageBoxW(nullptr, L"eBox已经运行", L"eBox", MB_OK);
...
unsigned long long login(handle_t /*IDL_handle*/, unsigned int pid, unsigned long long envFlag)
{
	try
	{
		...
		biz::env_logger().append(pEnv->getIndex(), biz::EnvLogType::Login, biz::EnvLogStatus::Success,
		                         L"进程接入环境", std::format(L"PID {} 已加入 环境{}", pid, pEnv->getIndex()));
