...
namespace
{
	...
	// ---------------------------------------------------------------------------
	// 激活状态持久化：HKCU\Software\eBox\License = 激活码文本
	// 兼容老版本：未命中新键时回退读 HKCU\Software\2Box\License
	// ---------------------------------------------------------------------------
	...
	std::wstring loadStoredCode()
	{
		...
		// 兼容老版本：新键未命中则读旧键 Software\2Box
		if (code.empty())
		{
			size = sizeof(buf);
			buf[0] = L'\0';
			if (RegOpenKeyExW(HKEY_CURRENT_USER, kLegacyRegSubKey, 0, KEY_READ, &hKey) == ERROR_SUCCESS)
			{
				if (RegQueryValueExW(hKey, kRegValue, nullptr, nullptr,
				                     reinterpret_cast<LPBYTE>(buf), &size) == ERROR_SUCCESS)
				{
	...
	// ---------------------------------------------------------------------------
	// 解绑次数持久化：HKCU\Software\eBox\UnbindMonth(REG_SZ "yyyyMM") + UnbindCount(REG_DWORD)
	...
	int loadUnbindCount()
	{
		...
		if (!hit)
		{
			month[0] = L'\0'; monthSize = sizeof(month);
			if (RegOpenKeyExW(HKEY_CURRENT_USER, kLegacyRegSubKey, 0, KEY_READ, &hKey) == ERROR_SUCCESS)
			{
				if (RegQueryValueExW(hKey, kRegUnbindMonth, nullptr, nullptr,
				                     reinterpret_cast<LPBYTE>(month), &monthSize) == ERROR_SUCCESS)
	...
	// 统计除当前进程外的 eBox.exe 实例数（解绑前必须关闭其他进程）
	// 兼容老版本：同时识别 2Box.exe（老版本进程）
	int countOtherBoxProcesses()
	{
		...
		if (Process32FirstW(hSnap, &pe))
		{
			do
			{
				...
				if (_wcsicmp(pe.szExeFile, L"eBox.exe") == 0 ||
				    _wcsicmp(pe.szExeFile, L"2Box.exe") == 0)
				{
					++count;
				}
